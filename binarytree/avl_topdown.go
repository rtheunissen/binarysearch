...
func (tree *AVLTopDown) Clone() list.List {
   return &AVLTopDown{Tree: tree.Tree.Clone()}
}

func (tree *AVLTopDown) Verify() {
   ...
   for {
      if i <= (*p).sizeL() {
         ...
         if (*p).l == nil {
            ...
            *g = tree.balanceDeleteL(*g)
            *g = tree.balanceDeleteR(*g)
...
func (tree *AVLTopDown) balanceInsertL(p *Node) *Node {
   //
...
// Symmetric
func (tree *AVLTopDown) balanceInsertR(p *Node) *Node {
...
func (tree *AVLTopDown) delete(p *Node, i list.Position, x *list.Data) *Node {
   tree.persist(&p)
   if i == p.sizeL() {
      *x = p.x
      defer tree.free(p)
...
func (tree *AVLTopDown) balanceDeleteL(p *Node) *Node {
   //
...
// Symmetric
func (tree *AVLTopDown) balanceDeleteR(p *Node) *Node {
   if tree.isTwoTwo(p) {
      tree.demote(p)
      return p
   }
...
func (tree *AVLTopDown) deleteMin(p *Node, min **Node) *Node {
   tree.persist(&p)
   if p.l == nil {
      *min = p
      return p.r
...
func (tree *AVLTopDown) deleteMax(p *Node, max **Node) *Node {
   tree.persist(&p)
   if p.r == nil {
      *max = p
      return p.l
...
//
func (tree *AVLTopDown) joinL(l, r *Node, sl list.Size) (p *Node) {
   if tree.rankDifference(r, l) <= 1 {
...
// Symmetric
func (tree *AVLTopDown) joinR(l, r *Node, sl list.Size) (p *Node) {
   if tree.rankDifference(l, r) <= 1 {
...
// Constructs a balanced tree with root `p` where all nodes in `l` are to the
// left of `p` and all nodes in `r` to the right of `p`.
func (tree *AVLTopDown) join(l, r *Node, sl list.Size) (p *Node) {
   if l == nil { return r }
   if r == nil { return l }
   if tree.rank(l) < tree.rank(r) {
...
func (tree *AVLTopDown) Split(i list.Position) (list.List, list.List) {
   ...
   return &AVLTopDown{Tree: Tree{arena: tree.arena, root: l, size: i}},
      &AVLTopDown{Tree: Tree{arena: tree.arena, root: r, size: tree.size - i}}
}
