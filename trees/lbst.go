...
func (LBST) isBalanced(x, y list.Size) bool {
   return utility.GreaterThanOrEqualToMSB(x, y >> 1)
}

func (LBST) singleRotation(x, y list.Size) bool {
   return utility.GreaterThanOrEqualToMSB(x, y)
}

func (tree LBST) join(l *Node, r *Node, sl, sr list.Size) (k *Node) {
   if l == nil { return r }
   if r == nil { return l }
   if sl <= sr {
...
func (tree LBST) extractMin(p *Node, s list.Size, x **Node) *Node {
   tree.persist(&p)
   if p.l == nil {
      *x = p
      p = p.r
...
func (tree LBST) extractMax(p *Node, s list.Size, x **Node) *Node {
   tree.persist(&p)
   if p.r == nil {
      *x = p
      p = p.l
...
func (tree LBST) build(l, p, r *Node, sl, sr list.Size) *Node {
...
func (tree *LBST) buildL(p *Node, l, r *Node, sl, sr list.Size) *Node {
...
func (tree *LBST) buildR(p *Node, l, r *Node, sl, sr list.Size) *Node {
...
func (tree *LBST) split(p *Node, i, s list.Size) (l, r *Node) {
   if p == nil {
      return
   }
   tree.persist(&p)
...
func (tree *LBST) Split(i list.Position) (LBST, LBST) {
