...
type Randomized struct {
   Tree
   random2.Source // compare performance vs making this directly xoshiro
}

func (Randomized) New() List {
   return &Randomized{
      Source: random2.New(random2.Uint64()),
   }
}
...
func (tree *Randomized) Insert(i Position, x Data) {
   ...
   tree.size++
   for {
      if random2.LessThan(s+1, tree.Source) == s {
...
func (tree *Randomized) join(l *Node, r *Node, sl, sr Size) (root *Node) {
   ...
   for {
      ...
      if random2.LessThan(sl+sr, tree.Source) < sl {
         tree.persist(&l)
         sl = sl - l.s - 1
         *p = l
         p = &l.r
