...
//
func (tree *LBSTRelaxed) delete(p **Node, s list.Size, i list.Size) (x list.Data) {
   for {
      tree.persist(p)
      sl := (*p).sizeL()
      sr := (*p).sizeR(s)
...
func (tree *LBSTRelaxed) Delete(i list.Position) list.Data {
 assert(i < tree.size)
 x := tree.delete(&tree.root, tree.size, i)
 tree.size = tree.size - 1
 return x
}
func (tree *LBSTRelaxed) insert(p **Node, s list.Size, i list.Position, x list.Data) {
...
func (tree *LBSTRelaxed) Insert(i list.Position, x list.Data) {
   assert(i <= tree.size)
   tree.size = tree.size + 1
...
//
//   height is height + 1 ?
func (tree *LBSTRelaxed) tooDeep(size list.Size, height uint64) bool {
   return (1 << ((height + 1) >> 1)) > size
}

func (tree *LBSTRelaxed) balance(p *Node, s list.Size) *Node {
   if s <= 3 {
      return p
   }
...
func (tree *LBSTRelaxed) rebuild(p **Node, s list.Size) {
   *p = tree.balance(*p, s)
}


func (tree *LBSTRelaxed) Split(i list.Size) (list.List, list.List) {
   assert(i <= tree.size)
...
func (tree *LBSTRelaxed) joinLr(l, r *Node, sl, sr list.Size) *Node {
   if r == nil {
      return l
   }
...
func (tree *LBSTRelaxed) join(l, r *Node, sl, sr list.Size) *Node {
   if sl > sr {
...
func (tree *LBSTRelaxed) Join(other list.List) list.List {
   tree.share(tree.root)
